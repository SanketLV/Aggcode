...
- **OpenCode has two ways in.** "Use OpenCode as installed" (`local`) takes no credentials: it stores `authMethod: "local"` on the `opencode` `ProviderConfig` row and counts as signed in, so chat runs on the free models plus whatever is already connected to OpenCode. The API-key method (`connect`) is unchanged. `resolveOpenCodeStatus` decides signed-in from the two inputs; a connected provider or the opt-in is enough. Full sign-out clears the opt-in, dropping one provider chip does not, and neither touches `auth.json`. The model list is filtered to providers OpenCode reports as connected (`openCodeModels.ts`), and a model ref splits on the first `/` only because OpenRouter ids contain slashes.
- **A sign-in method with no fields still needs a button.** `methodKind` in `lib/authMethod.ts` classifies a method as `fields`, `oauth` or `direct`; the modal used to render only a note for `direct`, which made a no-input method unclickable.
